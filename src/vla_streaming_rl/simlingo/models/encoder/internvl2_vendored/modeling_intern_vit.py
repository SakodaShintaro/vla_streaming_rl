...
class InternVisionEncoder(nn.Module):
    ...
    def __init__(self, config: InternVisionConfig):
        ...
        # simlingo-vendor-fix: pin device='cpu' so this works under transformers
        # 5.x, which wraps model __init__ in a `with torch.device("meta"):`
        # context. Without an explicit device, torch.linspace returns meta
        # tensors and .item() raises "cannot be called on meta tensors".
        dpr = [
            x.item()
