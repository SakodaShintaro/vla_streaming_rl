...
def get_assistant_loss_mask(user_starts, assistant_starts, prompt_tokenized_ids):
    ...
    loss_mask = torch.zeros(
        prompt_tokenized_ids.shape, dtype=torch.bool
    )  # loss is calculated where this mask is True

    for batch_id, (user_list, assistant_list) in enumerate(zip(user_starts, assistant_starts)):
        # batch_pairs = []
        # assume we start with user always:
        assert user_list[0] < assistant_list[0], (
            "First user start should be before first assistant start"
        )
        assert len(user_list) == len(assistant_list), (
            "Number of user and assistant starts should be the same"
        )
        ...
        for i, start in enumerate(assistant_list):
            # End is the start of the next user sequence OR the last index if it's the final sequence
                ...
                user_list[i + 1] - 1 if i < len(user_list) - 1 else seq_length - 1
            )  # updated variable name to seq_length
...
def get_chat_tokens(
    ...
    matches_user = (prompt_tokenized_ids.unfold(1, seq_len_to_find, 1) == user_start_token_ids).all(
        dim=2
    )
    matches_assistant = (
        prompt_tokenized_ids.unfold(1, seq_len_to_find_assistant, 1) == assistant_start_token_ids
    ).all(dim=2)
    # Get all matches
    ...
    loss_mask = get_assistant_loss_mask(
        position_user_start_indices, position_assistant_start_indices, prompt_tokenized_ids
    )
...
def get_custom_chat_template(
    ...
    for idx, conv in enumerate(conversations):
        assert len(conv) == 2, (
            "For question and answer templates only two turn conversation (user + assistant) is supported. During training is should work but is not checked!!"
        )
        ...
        assert conv[0]["role"] == "user", (
            "First turn should be user as this should be the question."
        )
        ...
        system_prompt = (
            template_conv.system_template.replace("{system_message}", template_conv.system_message)
            + template_conv.sep
        )
    ...
    conv_dict = get_chat_tokens(
        tokenizer, prompts_conv, user_start_token_str, assistant_start_token_str
    )
    question_dict = get_chat_tokens(
        tokenizer, prompts_question, user_start_token_str, assistant_start_token_str
    )
...
def preprocess_image_batch(images_batch_list, input_size=448, use_global_img=False, max_num_grid=2):
    ...
    for idx, img in enumerate(images_batch_list):
        ...
        images = dynamic_preprocess(
            image, image_size=input_size, use_thumbnail=use_global_img, max_num=max_num_grid
        )
