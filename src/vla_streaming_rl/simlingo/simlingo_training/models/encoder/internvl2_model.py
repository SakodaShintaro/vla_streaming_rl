...
class LingoInternVLModel(nn.Module):
    ...
    def replace_placeholder_tokens(
        self,
        adaptor_dict: torch.LongTensor,
        pixel_values: torch.FloatTensor,
        placeholder_values: List[dict],
        ...
        if pixel_values is not None and input_ids.shape[1] != 1 and pixel_values.size(0) > 0:
            ...
            for pixel_values_tmp in all_pixel_values:
                ...
                if pixel_values_tmp.dim() == 5:
                    ...
                    raise ValueError(
                        f"pixel_values of shape {pixel_values_tmp.shape}, expect to be of 4 or 5 dimensions"
                    )
            ...
            try:
                inputs_embeds[selected] = inputs_embeds[selected] * 0.0 + vit_embeds.reshape(
                    -1, C_embed
                )
