...
class InternVisionEncoder(nn.Module):
    ...
    def __init__(self, config: InternVisionConfig):
        super().__init__()
        self.config = config
        # stochastic depth decay rule (pure-python to avoid .item() on meta
        # tensors under transformers>=5 lazy/meta-device init)
        _n = config.num_hidden_layers
        dpr = [config.drop_path_rate * i / max(1, _n - 1) for i in range(_n)]
