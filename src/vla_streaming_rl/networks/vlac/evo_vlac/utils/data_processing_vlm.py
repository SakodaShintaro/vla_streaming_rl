...
def format_songling_v2(action, state=False, data_type=None):
    if action is None:
        ...
        if len(action) == 2:
            if state:
                return "\nleft: <position> {{x: {}mm, y: {}mm, z: {}mm, roll: {} degrees, pitch: {} degrees, yaw: {} degrees, open: {}}} </position> \nright: <position> {{x: {}mm, y: {}mm, z: {}mm, roll: {} degrees, pitch: {} degrees, yaw: {} degrees, open: {}}} </position>".format(
                    action[0][0],
                    action[0][1],
                    action[0][2],
                    action[0][3],
                ...
                return "\nleft: <action> {{x: {}mm, y: {}mm, z: {}mm, roll: {} degrees, pitch: {} degrees, yaw: {} degrees, open: {}}} </action> \nright: <action> {{x: {}mm, y: {}mm, z: {}mm, roll: {} degrees, pitch: {} degrees, yaw: {} degrees, open: {}}} </action>".format(
                    action[0][0],
                    action[0][1],
                    action[0][2],
                    action[0][3],
            ...
            return "{{x: {}mm, y: {}mm, z: {}mm, roll: {} degrees, pitch: {} degrees, yaw: {} degrees, open: {}}}".format(
                action[0][0],
                action[0][1],
                action[0][2],
                action[0][3],
...
def format_songling_simple(action, state=False, data_type=None):
    if action is None:
        ...
        if len(action) == 2:
            if state:
                return "\nleft: <(position)> ({} {} {} {} {} {} {}) </(position)> \nright: <(position)> ({}, {}, {}, {}, {}, {}, {}) </(position)>".format(
                    action[0][0],
                    action[0][1],
                    action[0][2],
                    action[0][3],
...
def bridge_position_preprocess(one=None, data=None, key=None, td=1, action=True):
    ...
    if type(position) is list:
        processed_position = [
            max(min(int(position[i] * 1000), 999), -999)
            if i < 3
...
def droid_process_single(action, action_type="action"):
    ...
    if type(action) is list:
        processed_action = [
            max(min(int(action[i] * 1000), 999), -999) if i < 3 else int(action[i] * 360 / 3.1416)
            for i in range(6)
        ]
