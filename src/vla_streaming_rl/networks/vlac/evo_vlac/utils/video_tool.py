    ...
    try:
        ...
        for i in range(len(image_objects)):
            ...
            if done_list and value_idx < len(done_list):
                ...
                done_color = (
                    "green"
                    if done_status == "Yes" or done_status == True or done_status == 1
                    else "red"
                )
            ...
            if curves_count > 0 and value_values:
                    ...
                    if current_value_idx >= 0:
                        ...
                        for j in range(current_value_idx):
                            color = (
                                ...
                                if (
                                    critic_values
                                    and j < len(critic_values)
                                    and critic_values[j] < 0
                                )
                                else "green"
                            )
                            ax.plot(
                                [j, j + 1],
                                [value_values[j], value_values[j + 1]],
                                color=color,
                                alpha=0.7,
                            )
                            marker_color = (
                            ...
                            ax.plot(
                                j, value_values[j], marker="o", color=marker_color, markersize=2
                            )
                        ...
                        current_color = (
                            ...
                            if (
                                critic_values
                                and current_value_idx < len(critic_values)
                                and critic_values[current_value_idx] < 0
                            )
                            else "green"
                        )
                        ax.plot(
                            current_value_idx,
                            value_values[current_value_idx],
                            marker="o",
                            color=current_color,
                            markersize=4,
                        )
                ...
                if done_values is not None:
                    ...
                    if current_value_idx >= 0:
                        ax.plot(
                            done_values[: current_value_idx + 1],
                            "b-",
                            marker="o",
                            markersize=2,
                            label="Done",
    ...
    try:
        ...
        for i in range(len(image_objects)):
            ...
            if value_idx >= 0:
                ...
                plt.ylim(
                    min(value_values) - abs(max(value_values)) * 0.15,
                    max(value_values) + abs(max(value_values)) * 0.15,
                )
                current_value_idx = min(value_idx, len(value_values) - 1)
                if current_value_idx >= 0:
                    for j in range(current_value_idx):
                        ax.plot(
                            ...
                            color="white",
                            alpha=0.3,
                            linewidth=line_width + 2 * edge_width,
                        )
                        color = (
                            "#4B0082"
                            if (critic_values and j < len(critic_values) and critic_values[j] < 0)
                            else "#FF4500"
                        )
                        ax.plot(
                            [j, j + 1],
                    ...
                    buf = fig.canvas.buffer_rgba()
                    w, h = fig.canvas.get_width_height()
                    plot_img = Image.frombuffer(
                        "RGBA", (w, h), buf, "raw", "RGBA", 0, 1
                    )  # .convert('RGB')
                    plt.close(fig)
...
def read_data_and_create_video(
    filename,
    camera_name=None,
    create_video=False,
    output_path=None,
