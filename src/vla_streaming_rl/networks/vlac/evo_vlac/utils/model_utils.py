...
from .data_processing_vlm import DataProcessor, denormalize_with_params, trojectory_example_prompt
...
class GAC_model:
    ...
    def get_action_score_prompt(self, task, state, action):
        ...
        action_score_prompt = self.dataclient.prompt_templete["task_action_score"].format(
            format_fuction(state), format_fuction(action), task
        )
    ...
    def get_action_prompt(
        ...
        full_prompt = image_prompt + self.dataclient.prompt_templete[action_key].format(
            format_fuction(state, state=position_output), task
        )
        if output_num > 1:
    ...
    def get_fast_action_prompt(
        ...
        full_prompt = self.get_action_prompt(
            task, view_num, position_output, simple, state, output_num, think
        )
        full_prompt += "<chunks>"
        return full_prompt
    ...
    def fast_results_format(
        ...
        for i in range(len(response_list)):
            ...
            fast_chunk = tokenizer.fasttokenizer.decode(
                temp, time_horizon=time_horizon, action_dim=action_dim
            )
            infer_requests[i].messages.append(
                {"role": "assistant", "content": response_list[i].choices[0].message.content}
            )
    ...
    def get_logprobs(self, infer_requests, return_mask=False, digit=False):
        ...
        mini_batch_encoded_inputs = [
            self.template.encode(infer_request) for infer_request in infer_requests
        ]
        ...
        if return_mask:
            if digit:
                ...
                digit_completion_mask = torch.zeros_like(
                    labels[:, -logits_to_keep:], dtype=torch.bool
                )
                    ...
                    for j in range(labels.shape[1] - logits_to_keep, labels.shape[1]):
                        if labels[i, j] != -100:
                            ...
                            if re.search(r"[0-9+\-]", token_str):
                                digit_completion_mask[i, j - (labels.shape[1] - logits_to_keep)] = (
                                    True
                                )
                return (
                    per_token_logps,
                    mini_batch_encoded_inputs["completion_mask"],
                    digit_completion_mask,
                )
            else:
    ...
    def get_in_context_done(
        self,
        task: str,
        first_image: List[Image.Image],
        n_pre_image: List[Image.Image],
        now_image: List[Image.Image],
        ref_image_list: List[Image.Image],
        ref_num=9,
        rich=False,
    ):
        """
        ...
        for one in answers_list:
            ...
            think_critic_list.append(
                "{:.1f}".format((100 - float(one_pre_value)) * float(one_critic) / 100.0)
            )
    ...
    def web_trajectory_done(
        self, task_description, main_image_path, reference_image_path=None, rich=False
    ):
        done_list = self.get_trajectory_done(
            task=task_description, image_list=[main_image_path], batch_num=1, rich=rich
        )
        ...
        if ref_image_list is None and goal_image is None:
            for i in tqdm.tqdm(range(len(image_list)), desc="done processing"):
                ...
                if (i + 1) % batch_num == 0 or i == len(image_list) - 1:
                    infer_requests = self.get_infer_requests(
                        prompt=batch_prompt, images=batch_image
                    )
                    response_list, infer_time = self.chat(infer_requests)
                    answers_list, complete_requests_list = self.results_format(
                        response_list, infer_requests, rich=rich
            ...
            for i in tqdm.tqdm(range(1, len(image_list)), desc="done processing"):
                ...
                if (i + 1) % batch_num == 0 or i == len(image_list) - 1:
                    batch_done, batch_related_critic = self.get_in_context_done(
                        task=task,
                        first_image=first_image,
                        n_pre_image=n_pre_image,
                        now_image=now_image,
    ...
    def magic_smooth(
        ...
        from video_tool import images_get_from_video, read_data_and_create_video
        ...
        image_list = read_data_and_create_video(
            file_path,
            camera_name=None,
            create_video=False,
            output_path=None,
            ...
            if float(critic_list[i]) <= 0:
                ...
                while i + skip_window < len(critic_list) and skip_window <= max_skip:
                    two_image = [
                        image_list[0],
                        image_list[i * skip],
                        image_list[(i + skip_window) * skip],
                    ]
                    one_critic_list, _ = self.get_trajectory_critic(
                        task=task,
                        image_list=two_image,
                        ref_image_list=ref_image_list,
                        batch_num=10,
                        ref_num=ref_num,
                        think=think,
        ...
        file_name = os.path.splitext(os.path.splitext(os.path.basename(file_path))[0])[
            0
        ]  # 去掉.pkl.gz
    ...
    def magic_split(
        self, task, file_path, hz=10, value_simple="mix_f", done_assist=False, ref_image_list=None
    ):
        import gzip
        import os
        import pickle
        ...
        if done_assist:
            ...
            while index < len(peak_list) - 1:
                if (
                    assist_peaks[peak_list[index + 1]][0] - assist_peaks[peak_list[index]][-1]
                    <= gap_merge_len
                ):
        ...
        for i, segment in enumerate(segments):
            ...
            task_name = (
                self.get_trajectory_task(segment["images"])[0]
                .split("</task>")[0]
                .strip()
                .replace(" ", "_")
                .replace(".", "_")
