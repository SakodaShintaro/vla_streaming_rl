...
def adaptive_peak_valley_detection(
    ...
    if assist_peaks:
        ...
        initial_valleys = [
            int((assist_peaks[k] + assist_peaks[k - 1]) / 2) for k in range(1, len(assist_peaks))
        ]
    else:
        ...
        if max_lag > min_lag:
            ...
            if len(peaks_auto) > 0:
                ...
                for i in range(1, min(50, len(search_autocorr) - 1)):
                    if (
                        search_autocorr[i] > search_autocorr[i - 1]
                        and search_autocorr[i] > search_autocorr[i + 1]
                        and search_autocorr[i] > 0.05
                    ):
                        period = i + min_lag
                        window_size = min(max(period // 2, 5), n // 4)
                        break
...
def _rolling_window_detection(
    data: np.ndarray, window_size: int, min_distance: int, prominence_threshold: float
    ...
    for i in range(half_window, n - half_window):
        ...
        # 检测峰值
        if (
            current_value == window_max
            and current_value - window_min > threshold
            and (len(peaks) == 0 or i - peaks[-1] >= min_distance)
        ):
            peaks.append(i)

        # 检测谷值
        elif (
            current_value == window_min
            and window_max - current_value > threshold
            and (len(valleys) == 0 or i - valleys[-1] >= min_distance)
        ):
            valleys.append(i)
...
def _iterative_peak_valley_optimization(
    ...
    for iteration in range(max_iterations):
        ...
        # Check convergence
        if np.array_equal(peaks, old_peaks) and np.array_equal(valleys, old_valleys):
            if verbose:
    ...
    peaks, valleys = _postprocess_amplitude_filtering(
        data, peaks, valleys, outlier_method, outlier_sensitivity, min_amplitude_ratio, verbose
    )
...
def _postprocess_amplitude_filtering(
    ...
    if verbose and len(all_outlier_indices) > 0:
        ...
        print(
            f"Detected {len(small_amplitude_indices)} small amplitude pairs (< {min_amplitude:.3f})"
        )
...
def _calculate_peak_valley_amplitudes(
    data: np.ndarray, peaks: np.ndarray, valleys: np.ndarray
...
def _detect_amplitude_outliers(
    amplitudes: np.ndarray, method: str, sensitivity: float, verbose: bool
) -> np.ndarray:
    """
    检测振幅中的离群点（差值过小的点）
    ...
    if method == "iqr":
        ...
        if verbose:
            print(
                f"Percentile method: {threshold_percentile:.1f}th percentile = {threshold_value:.3f}"
            )
...
def _remove_outlier_peak_valley_pairs(
    ...
    for idx in outlier_indices:
        if idx < len(peak_valley_pairs):
            ...
            if verbose:
                print(
                    f"Marking for removal - Peak: {peak_idx}, Valley: {valley_idx}, "
    ...
    if verbose:
        print(
            f"After outlier removal: Peaks {len(peaks)} -> {len(new_peaks)}, "
...
def _merge_adjacent_peaks(
    data: np.ndarray, peaks: np.ndarray, valleys: np.ndarray, verbose: bool
) -> np.ndarray:
    """
    合并相邻的峰点（当它们之间没有谷点时）
    """
    ...
    while i < len(all_points):
        if all_points[i][1] == "peak":
            ...
            if len(consecutive_peaks) > 1:
                ...
                if verbose:
                    print(
                        f"Merged {len(consecutive_peaks)} consecutive peaks, "
...
def _final_peak_valley_validation(
    ...
    if verbose:
        print(
            f"Final validation: Peaks {len(peaks)} -> {len(final_peaks)}, "
    ...
    for i in range(len(peaks_sorted) - 1):
        ...
        if len(between_valleys) == 0:
            ...
            if search_start < search_end:
                ...
                if verbose:
                    print(
                        f"Added missing valley at index {min_idx} between peaks {left_peak} and {right_peak}"
                    )
            ...
            if verbose:
                print(
                    f"Multiple valleys between peaks {left_peak} and {right_peak}, "
...
def _generate_segments(
    peaks: np.ndarray, valleys: np.ndarray, data_length: int
) -> List[Tuple[int, int]]:
    """生成序列分段"""
    all_points = []
    for p in peaks:
...
def plot_results(
    data: np.ndarray, result: Dict, title: str = "Peak Valley Detection", save_path: str = None
):
    ...
    if len(valleys) > 0:
        ...
        if len(valid_valleys) > 0:
            plt.plot(
                valid_valleys,
                [data[k] for k in valid_valleys],
                "go",
                markersize=8,
                label=f"Valleys ({len(valid_valleys)})",
            )
