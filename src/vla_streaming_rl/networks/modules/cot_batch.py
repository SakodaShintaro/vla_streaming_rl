...
from .vlm_inputs import build_video_media
...
class CoTBatch:
    ...
    def _write_chain(self) -> None:
        ...
        text = self.processor.apply_chat_template(
            ...
            tokenize=False,
            # Thinking off: the template then renders a closed, empty block and
            # the reply starts after it, which is where the protocol asks for the
            # reasoning. Left open, the chain reasons about the request instead
            # of about the scene.
        ...
        inputs = self.processor(
            text=[text],
            **build_video_media(
                self.processor, self.prompt_builder.frames(), self.prompt_builder.decision_fps
            ),
            return_tensors="pt",
            do_rescale=False,
        ).to(self.device)
