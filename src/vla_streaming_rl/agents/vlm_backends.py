...
class LocalVLMBackend:
    ...
    def __init__(
        ...
        *,
        ...
        self.model.eval()
        self.max_new_tokens = max_new_tokens
        # The chat template's own thinking block, which duplicates the <think>
        # the protocol asks for; the hosted backend caps it the same way.
    ...
    def generate(self, messages: list[dict]) -> VLMResponse:
        ...
        # response is what sits past the prompt length.
        completion_ids = generated[0, prompt_tokens:]
        completion_tokens = int(completion_ids.shape[0])
        return VLMResponse(
            text=self.processor.decode(completion_ids, skip_special_tokens=True),
            # There is no server to report one, so it is read off the budget the
            # same way an OpenAI-compatible endpoint decides it.
            finish_reason="length" if completion_tokens >= self.max_new_tokens else "stop",
