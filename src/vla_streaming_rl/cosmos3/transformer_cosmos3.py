...
#
# Vendored from huggingface/diffusers (main):
#   src/diffusers/models/transformers/transformer_cosmos3.py
# Two edits for the installed diffusers 0.35.2 (no git build pulled):
#   1. diffusers-internal relative imports rewritten to absolute `diffusers.*`.
#   2. the `parallel_config=` kwarg dropped from `dispatch_attention_fn` calls
#      (that argument only exists on the diffusers main branch).
# Model code is otherwise unmodified.
#
